...
# Load preprocessed dataset for time-series analysis
df = pd.read_csv("processed_layoff_data.csv")

# Convert 'Separation Date' to datetime for time-series analysis
...
# Streamlit UI
st.title("🔍 Employee Layoff Prediction with Time Series Analysis")

# Time-Series Analysis
st.header("📅 Time-Series Trends of Layoffs")

# Show monthly layoffs trend
st.subheader("Monthly Layoffs Trend")
st.line_chart(monthly_layoffs)

# Show yearly layoffs trend
st.subheader("Yearly Layoffs Trend")
st.line_chart(yearly_layoffs)

# Optional: Use Matplotlib for detailed plots (if needed)
st.subheader("Detailed Monthly Layoffs Trend")
fig, ax = plt.subplots(figsize=(10, 6))
monthly_layoffs.plot(kind='line', marker='o', ax=ax)
ax.set_title('Monthly Layoffs Trend')
ax.set_xlabel('Year-Month')
ax.set_ylabel('Number of Layoffs')
ax.set_xticklabels(monthly_layoffs.index, rotation=45)
st.pyplot(fig)

st.subheader("Detailed Yearly Layoffs Trend")
fig, ax = plt.subplots(figsize=(10, 6))
yearly_layoffs.plot(kind='line', marker='o', color='orange', ax=ax)
ax.set_title('Yearly Layoffs Trend')
ax.set_xlabel('Year')
ax.set_ylabel('Number of Layoffs')
ax.set_xticklabels(yearly_layoffs.index, rotation=45)
st.pyplot(fig)

# User Inputs for Prediction
st.header("💼 Input Employee Data for Prediction")
